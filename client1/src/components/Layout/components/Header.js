...
import { faUser, faCartShopping, faChevronDown, faCaretDown, faBars, faMagnifyingGlass } from '@fortawesome/free-solid-svg-icons'
...
export default function Header() {
    ...
    useEffect(() => {
        ...
        return () => {
            searchIconElement.removeEventListener('click', handleClickSearchIconElement);
                        ...
                        <div role='button' className="dropdown1">
                            {isAuth
                                ? <>
                                    ...
                                    <FontAwesomeIcon icon={faCaretDown} style={{ paddingLeft: '4px'}} className='icon'/>
                                </>
                            ...
                            <ul className="dropdown1-menu">
                                {isAuth
                                    ? <>
                                        <li>
                                            <Link to="/user/profile" className="dropdown1-item-link">Profile</Link>
                                        </li>
                                        <li>
                                            <Link to="/user/orders" className="dropdown1-item-link">Orders</Link>
                                        </li>
                                        <li><hr className="dropdown1-divider"/></li>
                                        <li>
                                            <button onClick={() => handleLogout()} className="dropdown1-item-link">Log out</button>
                                        </li>
                                    </>
                                    : <>
                                        <li>
                                            <Link to="/register" className="dropdown1-item-link">Register</Link>
                                        </li>
                                        <li>
                                            <Link to="/login" className="dropdown1-item-link">Log in</Link>
                                        </li>
                                    </>
                                }
                            </ul>
                ...
                <ul className="nav-inner">
                    {
                        user?.role === 1 && (
                            <li className="nav-item">
                                <NavLink to="/admin" className="nav-item-link">Admin</NavLink>
                            </li>
                        )
                    }
                    <li className="nav-item">
                        <NavLink to="/" className="nav-item-link">Trang chủ</NavLink>
                    </li>
                    <li className="nav-item">
                        <label className="nav-item-link" htmlFor="drop-1">
                            Sản phẩm
                            <FontAwesomeIcon icon={faChevronDown} style={{paddingLeft: '4px'}}/>
                        </label>
                        <input type='checkbox' id="drop-1" />
                        <ul className="sub-menu">
                            {categories.map(c => (
                                <li key={c._id} className="sub-menu-item">
                                    <Link to={`/${c.slug}`} className="sub-menu-item-link">{c.name}</Link>
                                </li>
                            ))}
                        </ul>
                    </li>
                    <li className="nav-item">
                        <NavLink to="/about-us" className="nav-item-link">Về chúng tôi</NavLink>
                    </li>
                    <li className="nav-item">
                        <NavLink to="/help" className="nav-item-link">Trợ giúp</NavLink>
                    </li>
                </ul>
            </nav>   
