...
export default function EditProductModal(props) {
    const {handleClose, open, editedProduct, handleUpdateProductListFromEditModal} = props;
    ...
    useEffect(() => {
        if(open) {
            setUpdatedProduct({...editedProduct, categoryId: editedProduct.category._id})
        }
    }, [editedProduct])
    ...
    const handleSubmit = async () => {
        try {
            ...
            const { data } = await axios.patch(
                process.env.REACT_APP_API + `api/v1/product/update-product/${updatedProduct._id}`,
                productData,
                {
                    headers: {
    ...
    <Modal
        ...
        <form encType="multipart/form-data">
            ...
            <div className="mb-3">
                ...
                {updatedProduct?.photo ? updatedProduct?.photo.name : "Upload Photo"}
                <input
                    type="file"
                    name="photo"
                    value=""
                    accept="image/*"
                    onChange={(e) => {
                        setUpdatedProduct({...updatedProduct, photo: e.target.files[0]});
                    }}
                    hidden
                />
                </label>
                ...
                <div className="mb-3">
                    <div className="text-center">
                        <img
                            src={
                                !updatedProduct?.photo
                                ? process.env.REACT_APP_API + `api/v1/product/product-photo/${updatedProduct._id}`
            ...
            <Select
                ...
                onChange={(value) => setUpdatedProduct({ ...updatedProduct, shipping: value })}
            >
