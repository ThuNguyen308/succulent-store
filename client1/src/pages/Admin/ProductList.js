...
export default function ProductList() {
    ...
    try {
      const { data } = await axios.post(
        process.env.REACT_APP_API +
          `api/v1/product/get-paginated-products/${page}/${limit}`,
        {
          search,
          sortBy,
          sortField,
  ...
  const addMultiProduct = async (arrayProduct) => {
    try {
      const { data } = await axios.post(
        process.env.REACT_APP_API + `api/v1/product/create-multi-product`,
        { arrayProduct },
        {
          headers: {
      ...
      <div className="d-sm-flex justify-content-between">
        ...
        <div className="scrollable--without-scrollbar">
          <div className="group-btns">
            ...
            <input id="import" type="file" hidden onChange={getCsvFile} />
            <CSVLink
              data={dataExport}
